import LogoKhoaCNTT from "../assets/images/logo-khoa-cntt.png";
import LogoKhoaOTO from "../assets/images/logo-khoa-oto.png";
import LogoKhoaCOKHI from "../assets/images/logo-khoa-co-khi.png";
import LogoKhoaTHUCPHAM from "../assets/images/logo-khoa-thuc-pham.png";
import LogoKhoaDLNN from "../assets/images/logo-khoa-du-lich-ngoai-ngu.png";
import LogoKhoaDIEN from "../assets/images/logo-khoa-dien.png";
import LogoKhoaKINHTE from "../assets/images/logo-khoa-kinh-te.png";
import LogoTruong from "../assets/images/logo.png";
import MayImage from "../assets/images/may-img.png";
import CNTTImage from "../assets/images/cntt-img.png";
import OtoImage from "../assets/images/oto-img.png";
import CoKhiImage from "../assets/images/cokhi-img.png";
import ThucPhamImage from "../assets/images/thucpham-img.png";
import DienImage from "../assets/images/dien-img.png";
import DuLichImage from "../assets/images/dulich-img.png";
import KinhTeImage from "../assets/images/kinhte-img.png";
import RoboconImage from "../assets/images/robocon-img.jpg";
import LanhDaoTruongLQD from "../assets/images/ldtlqd-img.jpg";
import PhoHieuTruongDHSD from "../assets/images/phtdhsd-img.jpg";
import PhoTruongPhongCTSV from "../assets/images/ptpctsv-img.jpg";
import GiamDocSacombank from "../assets/images/giamdocsac-img.jpg";
import PhongChongCovit from "../assets/images/phongchongcovit-img.jpg";
import SinhVienRaTruong from "../assets/images/sinhvienratruong-img.jpg";
import News8 from "../assets/images/news8-img.jpg";
import TrongCay from "../assets/images/trongcay-img.jpg";
import ThuVienAnhKhoaCNTT from "../assets/images/thuvienanhkhoacntt-img.jpg";
import ThuVienAnhKhoaOto from "../assets/images/thuvienanhkhoaoto-img.jpg";
import ThuVienAnhKhoaKinhTe from "../assets/images/thuvienanhkhoakinhte-img.jpg";
import ThuVienAnhKhoaCoKhi from "../assets/images/thuvienanhkhoacokhi-img.jpg";
import ThuVienAnhKhoaThucPham from "../assets/images/thuvienanhkhoathucpham-img.jpg";
import ThuVienAnhKhoaDuLich from "../assets/images/thuvienanhkhoadulich-img.jpg";
import ThuVienAnhKhoaMay from "../assets/images/thuvienanhkhoamay-img.jpg";
import ThuVienAnhKhoaDien from "../assets/images/thuvienanhkhoadien-img.jpg";

export const SLIDES_DATA = [
  {
    id: "s1",
    title: "Nơi học tập",
    content:
      "Trường có chức năng đào tạo, bồi dưỡng nguồn nhân lực có trình độ đại học, sau đại học và các trình" +
      " độ thấp hơn theo quy định của pháp luật, đồng thời là trung tâm nghiên cứu, ứng dụng, chuyển giao " +
      "khoa học - công nghệ đa ngành, đa lĩnh vực, chất lượng cao, đạt trình độ tiên tiến trong hệ thống giáo " +
      "dục quốc dân, đáp ứng nhu cầu phát triển kinh tế, xã hội của đất nước.",
  },
  {
    id: "s2",
    title: "Nơi rèn luyện",
    content:
      "Hiện nay, trường đào tạo các hệ Đại học, Đại học liên thông, Cao đẳng, Cao đẳng liên thông, Trung cấp" +
      " chuyên nghiệp, Cao đẳng nghề, Trung cấp nghề,... trong nhiều lĩnh vực, chủ yếu là kinh tế, kỹ thuật" +
      " công nghiệp.Trường Đại học Sao Đỏ (tên tiếng Anh: Red Star University) là trường Đại học công lập được" +
      " thành lập năm 2010 trên cơ sở nâng cấp Trường Cao đẳng Công nghiệp Sao Đỏ.",
  },
  {
    id: "s3",
    title: "Nơi trải nghiệm",
    content:
      "Trường Đại học Sao Đỏ chịu sự quản lý, chỉ đạo trực tiếp của Bộ Công thương; sự quản lý nhà nước về" +
      " giáo dục và đào tạo của Bộ Giáo dục . Đào tạo, về dạy nghề của Bộ Lao động - Thương binh và Xã hội, " +
      " chịu sự chỉ đạo về các lĩnh vực nghiệp vụ của các Bộ, ngành liên quan; chịu sự quản lý hành chính theo" +
      " lãnh thổ của UBND tỉnh Hải Dương - nơi trường đặt trụ sở chính.",
  },
  {
    id: "s4",
    title: "Nơi giải trí",
    content:
      "Trường có chức năng đào tạo, bồi dưỡng nguồn nhân lực có trình độ đại học, sau đại học và các trình độ thấp hơn theo quy định của pháp luật; đồng thời là trung tâm nghiên cứu, ứng dụng, chuyển giao khoa học - công nghệ đa ngành, đa lĩnh vực, chất lượng cao, đạt trình độ tiên tiến trong hệ thống giáo dục quốc dân, đáp ứng nhu cầu phát triển kinh tế, xã hội của đất nước.",
  },
  // {
  //   id: "s5",
  //   title: "Nơi combat",
  //   content:
  //     "Trường có chức năng đào tạo, bồi dưỡng nguồn nhân lực có trình độ đại học, sau đại học và các trình" +
  //     " độ thấp hơn theo quy định của pháp luật; đồng thời là trung tâm nghiên cứu, ứng dụng, chuyển giao " +
  //     "khoa học - công nghệ đa ngành, đa lĩnh vực, chất lượng cao, đạt trình độ tiên tiến trong hệ thống giáo " +
  //     "dục quốc dân, đáp ứng nhu cầu phát triển kinh tế, xã hội của đất nước.",
  // },
];

export const INDUSTRYS_DATA = [
  {
    id: "i1",
    image: CNTTImage,
    industry: "Khoa công nghệ thông tin",
    title: "Với sự phát triển của công nghệ máy tính...",
  },
  {
    id: "i2",
    image: CoKhiImage,
    industry: "Khoa cơ khí",
    title: "Với sự phát triển của của các công nghệ hiện đại ..",
  },
  {
    id: "i3",
    image: KinhTeImage,
    industry: "Khoa kinh tế",
    title: "Kinh tế thế giới nói chung và kinh tế Việt Nam ...",
  },
  {
    id: "i4",
    image: DienImage,
    industry: "Khoa điện",
    title: "Ngành điện hiện đang là ngành vô cùng ...",
  },
  {
    id: "i5",
    image: MayImage,
    industry: "Khoa may và thời trang",
    title: "Công nghệ dệt may, thiết kế thười trang...",
  },
  {
    id: "i6",
    image: ThucPhamImage,
    industry: "Khoa thực phẩm",
    title: "Công nghệ thực phẩm đã đang được phát triển...",
  },
  {
    id: "i7",
    image: OtoImage,
    industry: "Khoa ô tô",
    title: "Kĩ thuật ô tô , máy móc hiện đại đã và đang...",
  },
  {
    id: "i8",
    image: DuLichImage,
    industry: "Khoa du lịch và ngoại ngữ",
    title: "Ngoại ngữ và du lịch đang là ngành vô cùng hot...",
  },
];

export const GAMEOPTIONS_DATA = [
  {
    id: "g1",
    image: LogoTruong,
    industry: "Trường đại học Sao Đỏ",
    details_content:
      "Trường Đại học Sao Đỏ (tên tiếng Anh: Red Star University) là trường Đại học công lập được thành lập năm 2010 trên cơ sở nâng cấp Trường Cao đẳng Công nghiệp Sao Đỏ, có trụ sở chính tại thành phố Chí Linh, tỉnh Hải Dương. Trường trực thuộc Bộ Công thương, Trường có chức năng đào tạo, bồi dưỡng nguồn nhân lực có trình độ đại học, sau đại học và các trình độ thấp hơn theo quy định của pháp luật; đồng thời là trung tâm nghiên cứu, ứng dụng, chuyển giao khoa học - công nghệ đa ngành, đa lĩnh vực, chất lượng cao, đạt trình độ tiên tiến trong hệ thống giáo dục quốc dân, đáp ứng nhu cầu phát triển kinh tế, xã hội của đất nước. Hiện nay, trường đào tạo các hệ Đại học, Đại học liên thông, Cao đẳng, Cao đẳng liên thông, Trung cấp chuyên nghiệp, Cao đẳng nghề, Trung cấp nghề,... trong nhiều lĩnh vực, chủ yếu là kinh tế, kỹ thuật công nghiệp.",
    content:
      "Trường có chức năng đào tạo, bồi dưỡng nguồn nhân lực có trình độ đại học, sau đại học và các trình độ thấp hơn theo quy định của pháp luật; đồng thời là trung tâm nghiên cứu, ứng dụng, chuyển giao khoa học - công nghệ đa ngành.",
  },
  {
    id: "g2",
    image: LogoKhoaCNTT,
    industry: "Khoa công nghệ thông tin",
    details_content:
      "Hiện nay, ngành công nghệ thông tin là một trong những ngành học được chú trọng trong hệ thống đào tạo của trường Đại học Sao Đỏ cũng như các trường Đại học khác có đào tạo ngành học này. Nó được xem là ngành đào tạo mũi nhọn hướng đến sự phát triển của công nghệ và khoa học kỹ thuật trong thời đại số hóa ngày nay. Công nghệ thông tin là một ngành học được đào tạo để sử dụng máy tính và các phần mềm máy tính để phân phối và xử lý các dữ liệu thông tin, đồng thời dùng để trao đổi, lưu trữ và chuyển đổi các dữ liệu thông tin dưới nhiều hình thức khác nhau.",
    content:
      "Hiện nay, ngành công nghệ thông tin là một trong những ngành học" +
      "được chú trọng trong hệ thống đào tạo của trường Đại học Sao Đỏ" +
      "cũng như các trường Đại học khác có đào tạo ngành học này. Nó" +
      "được xem là ngành đào mũi nhọn.",
  },
  {
    id: "g3",
    image: LogoKhoaCOKHI,
    industry: "Khoa cơ khí",
    details_content:
      "Ngành Kỹ thuật cơ khí là một ngành khoa học kỹ thuật chuyên ứng dụng những nguyên lý vật lý cùng với kỹ thuật và khoa học vật liệu để có thể thiết kế, phân tích, chế tạo và bảo dưỡng các loại máy móc và hệ thống cơ khí. Ngành Kỹ thuật cơ khí sẽ phục vụ cho công tác thiết kế trong các lĩnh vực như ô tô, máy bay, phương tiện giao thông, các hệ thống gia nhiệt và làm lạnh, đồ dùng gia đình, máy móc và thiết bị sản xuất, vũ khí. Đây là lĩnh vực kỹ thuật liên quan đến thiết kế, chế tạo và vận hành máy móc. ",
    content:
      "Ngành Kỹ thuật cơ khí là một ngành khoa học kỹ thuật chuyên ứng dụng những nguyên lý vật lý cùng với kỹ thuật và khoa học vật liệu để có thể thiết kế, phân tích, chế tạo và bảo dưỡng các loại máy móc và hệ thống cơ khí.",
  },
  {
    id: "g4",
    image: LogoKhoaOTO,
    industry: "Khoa ô tô",
    details_content:
      "Với sự phát triển không ngừng của khoa học kỹ thuật và nhu cầu xã hội, công nghệ ô tô trở thành lĩnh vực mũi nhọn của nền kinh tế toàn cầu. Cụ thể, ngành nghề này đang ngày càng cho thấy những tiềm năng lớn mạnh. Nhu cầu lớn về nguồn nhân lực: Trong xu thế hội nhập, nhu cầu mua sắm các loại phương tiện di chuyển đặc biệt là ô tô đang có tốc độ gia tăng nhanh chóng. Thực tế này kéo theo nhu cầu về nguồn nhân lực ngày một tăng trong tương lai.",
    content:
      "Với sự phát triển không ngừng của khoa học kỹ thuật và nhu cầu xã hội, công nghệ ô tô trở thành lĩnh vực mũi nhọn của nền kinh tế toàn cầu. Cụ thể, ngành nghề này đang ngày càng cho thấy những tiềm năng lớn mạnh.",
  },
  {
    id: "g5",
    image: LogoKhoaDIEN,
    industry: "Khoa điện",
    content:
      "Sinh viên tốt nghiệp ngành Kỹ thuật điện sẽ đảm nhận những công việc về thiết kế thiết bị điện và điện tử công suất, quản lý và vận hành lưới điện, thiết kế chiếu sáng, phân tích ổn định thiết bị và nguồn điện, bảo vệ relay.",
  },
  {
    id: "g6",
    image: LogoKhoaTHUCPHAM,
    industry: "Khoa thực phẩm",
    details_content:
      "Công nghệ thực phẩm, hiểu một cách đơn giản, là ngành học chuyên về lĩnh vực bảo quản, chế biến nông sản, kiểm tra, đánh giá chất lượng trong quá trình chế biến thực phẩm; đồng thời nghiên cứu phát triển các sản phẩm mới, vận hành dây chuyền sản xuất - bảo quản, tạo ra nguyên liệu mới trong lĩnh vực thực phẩm, dược phẩm hay hóa học… Ứng dụng của ngành này vô cùng đa dạng và gần gũi với đời sống hàng ngày của chúng ta. Công nghệ thực phẩm ngày càng chiếm vai trò quan trọng trong đời sống hàng ngày vì mức độ phức tạp trong nhu cầu của con người đang gia tăng mạnh. Cụ thể tại Việt Nam, với dân số hơn 90 triệu người và tốc độ tăng trưởng kinh tế trung bình hàng năm là 7,5%, nhu cầu tiêu dùng đối với thực phẩm chế biến ngày càng lớn và phong phú cả về mẫu mã lẫn chất lượng, đặc biệt là sản phẩm sạch.",
    content:
      "Công nghệ thực phẩm, hiểu một cách đơn giản, là ngành học chuyên về lĩnh vực bảo quản, chế biến nông sản, kiểm tra, đánh giá chất lượng trong quá trình chế biến thực phẩm; đồng thời nghiên cứu phát triển các sản phẩm mới.",
  },
  {
    id: "g7",
    image: LogoKhoaDLNN,
    industry: "Khoa du lịch & ngoại ngữ",
    details_content:
      "Ngành du lịch là ngành kinh tế tổng hợp, gồm nhiều nhóm ngành nghề liên quan đến nhiệm vụ đào tạo và cung cấp nhân lực cho các công ty lữ hành, khách sạn, hay nhà hàng,…Ngành du lịch có sự đóng góp to lớn vào tổng sản phẩm kinh tế quốc dân ở nước ta. Hiện du lịch được coi là một trong ba ngành kinh tế lớn được chú trọng đầu tư, không ngừng phát triển. Du lịch thúc đẩy hỗ trợ các ngành như giao thông vận tải, bưu chính viễn thông, bảo hiểm, dịch vụ tài chính, lưu trú và ăn uống, đồng thời giải quyết việc làm cho đa số lao động phụ ở các thành phố, thị trấn.",
    content:
      "Ngành du lịch là ngành kinh tế tổng hợp, gồm nhiều nhóm ngành nghề liên quan đến nhiệm vụ đào tạo và cung cấp nhân lực cho các công ty lữ hành, khách sạn, hay nhà hàng,…Ngành du lịch có sự đóng góp to lớn vào tổng sản phẩm.",
  },
  {
    id: "g8",
    image: LogoKhoaKINHTE,
    industry: "Khoa kinh tế",
    details_content:
      "Ngành Kinh tế hay Kinh tế học là môn khoa học xã hội nghiên cứu sự sản xuất, phân phối và tiêu dùng các loại hàng hóa và dịch vụ. Kinh tế học cũng nghiên cứu cách thức xã hội quản lý các nguồn tài nguyên (nguồn lực) khan hiếm của nó. Nghiên cứu Kinh tế học nhằm mục đích giải thích cách thức các nền kinh tế vận động và cách tác nhân kinh tế tương tác với nhau. Các nguyên tắc kinh tế được ứng dụng trong đời sống xã hội, trong thương mại, tài chính và hành chính công, thậm chí là trong ngành tội phạm học, giáo dục, xã hội học, luật học và nhiều ngành khoa học khác.",
    content:
      "Ngành Kinh tế hay Kinh tế học là môn khoa học xã hội nghiên cứu sự sản xuất, phân phối và tiêu dùng các loại hàng hóa và dịch vụ. Kinh tế học cũng nghiên cứu cách thức xã hội quản lý các nguồn tài nguyên (nguồn lực) khan hiếm của nó.",
  },
];

export const NEWS_DATA = [
  {
    id: "n1",
    image: RoboconImage,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title: "ĐỘI ROBOCON ĐẠI HỌC SAO ĐỎ",
    content:
      "Trường Đại học Sao Đỏ họp, đánh giá kết quả thi vòng loại robocon khu vực phía Bắc và triển khai kế hoạch vòng Chung kết Toàn quốc năm 2023",
  },
  {
    id: "n2",
    image: LanhDaoTruongLQD,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title: "Các hoạt động nhằm phát động...",
    content:
      "Phát biểu của lãnh đạo Trường THPT Lê Quý Đôn (Thanh Miện - Hải Dương) ông Nguyễn Hoàng Anh – Chủ tịch Hội đồng quản trị giới thiệu về quy mô đào",
  },
  {
    id: "n3",
    image: PhoHieuTruongDHSD,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title:
      "TS. Nguyễn Thị Kim Nguyên - Phó Hiệu trưởng Trường Đại học Sao Đỏ, ...",
    content:
      "Trường Đại học Sao Đỏ họp, đánh giá kết quả thi vòng loại robocon khu vực phía Bắc và triển khai kế hoạch vòng Chung kết Toàn quốc năm 2023",
  },
  {
    id: "n4",
    image: PhoTruongPhongCTSV,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title:
      "TS. Nguyễn Thị Nhan – Phó Trưởng phòng Công tác sinh viên báo cáo...",
    content:
      "Trường Đại học Sao Đỏ họp, đánh giá kết quả thi vòng loại robocon khu vực phía Bắc và triển khai kế hoạch vòng Chung kết Toàn quốc năm 2023",
  },
  {
    id: "n5",
    image: GiamDocSacombank,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title: "Bà Vũ Thị Thu Nga – Giám đốc Ngân hàng Sacombank ...",
    content:
      "Tại chương trình, sinh viên được nghe chuyên gia nhân sự chia sẻ về những kinh nghiệm thực tế,...",
  },

  {
    id: "n6",
    image: PhongChongCovit,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title: "Phòng Chống Covit ,Phòng chống...",
  },
  {
    id: "n7",
    image: SinhVienRaTruong,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title: "Khóa Sinh Viên Vừa mới ra trường...",
  },
  {
    id: "n8",
    image: News8,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title: "Trường Đại Học Sao Đỏ có những...",
  },
  {
    id: "n9",
    image: TrongCay,
    authName: "dangLong",
    authAvatar: null,
    view: 222,
    comment: 111,
    title: "TRồng Cây xanh góp phần...",
  },
];

export const LIBRARY_DATA = [
  {
    id: "i1",
    industry: "Khoa công nghệ thông tin",
    image: ThuVienAnhKhoaCNTT,
  },
  {
    id: "i2",
    industry: "Khoa ô tô",
    image: ThuVienAnhKhoaOto,
  },
  {
    id: "i3",
    industry: "Khoa kinh tế",
    image: ThuVienAnhKhoaKinhTe,
  },
  {
    id: "i4",
    industry: "Khoa cơ khí",
    image: ThuVienAnhKhoaCoKhi,
  },
  {
    id: "i5",
    industry: "Khoa thực phẩm",
    image: ThuVienAnhKhoaThucPham,
  },
  {
    id: "i6",
    industry: "Khoa du lịch và ngoại ngữ",
    image: ThuVienAnhKhoaDuLich,
  },
  {
    id: "i7",
    industry: "Khoa may",
    image: ThuVienAnhKhoaMay,
  },
  {
    id: "i8",
    industry: "Khoa điện",
    image: ThuVienAnhKhoaDien,
  },
];

export const NOTIFIACTIONS_DATA = [
  {
    id: "n1",
    content: "Thông báo kết quả trúng tuyển viên chức năm 2022.",
    path: null,
  },
  {
    id: "n2",
    content:
      "Thông báo kết quả xét tuyển vòng 1 và triệu tập thí sinh dự tuyển vòng 2 kỳ tuyển dụng viên chức năm 2022",
    path: null,
  },
  {
    id: "n3",
    content: "Kế hoạch tổ chức kỳ thi phụ học kỳ I, năm học 2021 - 2022",
    path: null,
  },
  {
    id: "n4",
    content:
      "Thông báo về việc sinh viên trở lại học tập trung tại Trường sau nghỉ Tết Nguyên đán Nhâm Dần - 2022",
    path: null,
  },
  {
    id: "n5",
    content:
      "Thông báo:  Tiêm liều bổ sung đối với mũi 1, mũi 2 Vaccine phòng Covid-19",
    path: null,
  },
];
